	...
	<table class="table table-hover" width="600" cellpadding="0" cellspacing="0" border="1" >
		<thead class="thead-dark">			
		 ...
		 <tbody align="center">
			...
			<c:forEach items="${searchmemberlist}" var="MemberDto">
			<c:choose>
		...
		<nav aria-label="Page navigation example">
		  <ul class="pagination justify-content-center">
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item">
						<a class="page-link" href ="admin_search.do?page=1&search=<%= session.getAttribute("csearch") %>&option=<%= session.getAttribute("cmenuchoice") %>"  tabindex="-1" aria-disabled="true">First</a>
					</c:otherwise>				
				</c:choose>			  			
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item">
						<a class="page-link" href ="admin_search.do?page=${page.curPage - 1}&search=<%= session.getAttribute("csearch") %>&option=<%= session.getAttribute("cmenuchoice") %>"  tabindex="-1" aria-disabled="true">Previous</a>
					</c:otherwise>	
				</c:choose>
		    ...
		    <li class="page-item">		   
				...
				<c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item">
							<a class="page-link" href ="admin_search.do?page=${fEach}&search=<%= session.getAttribute("csearch") %>&option=<%= session.getAttribute("cmenuchoice") %>"  tabindex="-1" aria-disabled="true">[${fEach}]&nbsp;</a>						
						</c:otherwise>			
					</c:choose>
				</c:forEach>		    
		    </li>
		    <li class="page-item">
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item">
						<a class="page-link" href ="admin_search.do?page=${page.curPage + 1}&search=<%= session.getAttribute("csearch") %>&option=<%= session.getAttribute("cmenuchoice") %>"  tabindex="-1" aria-disabled="true">Next</a>
					</c:otherwise>
				</c:choose>
		    </li>		    
		    ...
		    <li class="page-item">
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item">
						<a class="page-link" href ="admin_search.do?page=${page.totalPage}&search=<%= session.getAttribute("csearch") %>&option=<%= session.getAttribute("cmenuchoice") %>"  tabindex="-1" aria-disabled="true">End</a>
					</c:otherwise>
				</c:choose>		    
		  </ul>
		</nav>		
